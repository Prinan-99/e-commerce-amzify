...
import { chatWithLumina } from '../services/geminiService';
import { ChatMessage, GroundingSource } from '../types/types';

const AISearch: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [input, setInput] = useState('');
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "Hello! I'm Lumina, your personal luxury concierge. Looking for something specific or need some inspiration for a gift?" }
  ]);
  ...
  const handleSend = async () => {
    ...
    const result = await chatWithLumina(userText, messages);
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-40 flex flex-col items-end transition-all duration-300 ${isOpen ? 'w-full max-w-md' : 'w-auto'}`}>
      {isOpen ? (
        <div className="bg-white rounded-2xl shadow-2xl w-full border border-slate-200 overflow-hidden flex flex-col max-h-[600px] mb-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-2">
                  ...
                  <span className="text-sm text-slate-500">Lumina is thinking...</span>
                </div>
              </div>
            )}
      ...
      <button 
        ...
        className="bg-slate-900 text-white h-14 w-14 hover:w-52 transition-all duration-500 ease-in-out rounded-full shadow-2xl hover:shadow-indigo-500/30 hover:bg-slate-800 flex items-center justify-center group overflow-hidden"
      >
        <div className="flex items-center px-4 w-full">
          <Sparkles className="w-6 h-6 text-indigo-400 flex-shrink-0 group-hover:animate-pulse" />
          <span className="max-w-0 opacity-0 group-hover:max-w-xs group-hover:opacity-100 group-hover:ml-3 transition-all duration-500 ease-in-out whitespace-nowrap font-black text-[10px] uppercase tracking-widest pointer-events-none">
            Ask Lumina
          </span>
        </div>
      </button>
