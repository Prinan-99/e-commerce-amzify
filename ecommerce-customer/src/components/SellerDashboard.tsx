...
import { 
  Users, Plus, Search, Zap, Megaphone, Mail, Sparkles, Loader2,
  Download, Package, ToggleLeft, ToggleRight, RefreshCw
} from 'lucide-react';
import { PRODUCTS as initialProducts } from '../config/constants';
import { Order, SellerTab, AutomationRule, Customer, Campaign } from '../types/types';
import { 
  getSellerInsights, 
...
const SellerDashboard: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from({ length: points }).map((_, i) => {
      // Use a deterministic seed based on index to avoid random values during render
      const seed = i * 1000 + (metricType === 'revenue' ? 1 : 0);
      return {
        label: rangeMode === '1y' ? MONTH_NAMES[i % 12] : `${i + 1}`,
        value: metricType === 'revenue' 
          ? Math.floor((seed % 50000)) + 15000 
          : Math.floor((seed % 20)) + 5,
        active: i === points - 1
      };
    });
  ...
  const handleDeployCampaign = async () => {
    ...
    await generateMarketingCreative(mktProduct, mktGoal, 'Minimalist');
      ...
      <div className="max-w-[1440px] mx-auto p-6 md:p-10 space-y-8 md:space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-lg shadow-xl shadow-indigo-100">L</div>
          </div>
        </div>
