...
// Simulate backend OTP generation and sending.
// IMPORTANT: The admin sender email (amzify54@gmail.com) is NEVER present or exposed in frontend code.
// The backend is responsible for sending OTP emails from the admin address.
const mockSendOtp = async (contact: string) => {
...
const LoginPortal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      const resp = await mockSendOtp(contact);
      setOtpLoading(false);
      if (resp.success) {
        setOtpSent(true);
        setStep('otp');
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="bg-white/80 backdrop-blur-xl p-12 rounded-3xl border border-slate-200/50 shadow-2xl shadow-indigo-500/10 max-w-md w-full relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="absolute left-6 top-6">
          <button
            className="flex items-center justify-center p-2 bg-slate-100 hover:bg-indigo-50 text-indigo-700 rounded-xl text-xs shadow-sm border border-slate-200 transition-all relative group"
          ...
          >
            ...
            <span className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2 py-1 bg-slate-800 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-20">
              Back to Shopping
            </span>
          </button>
        </div>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                disabled={otpLoading || otpAttempts >= OTP_RULES.maxAttempts}
              />
            </div>
            {otpError && (
